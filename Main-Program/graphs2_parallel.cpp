...
int* findKShortestParallel(Graph2* graph, const string& startNodeName, const string& endNodeName, int k) {
    ...
    while (!pq.empty()) {
        ...
        for (size_t i = 0; i < edges.size(); ++i) {
            int v = nodeIndex(edges[i]->getName(), nodes);
            int weight = graph->getWeightBetweenNodes(nodes[u], edges[i]);
...
int main(int argc, char** argv) {
    ...
    if (!timeFile.is_open()) {
        ...
        int* shortestPaths = findKShortestParallel(graph, startNodeName, endNodeName, k);
